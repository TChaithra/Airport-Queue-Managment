...
OV_MODEL_CANDIDATES = [
   # _SCRIPT_DIR / "topview-dataset_int8_openvino_model"     / "topview-dataset.xml", 
...
# ─────────────────────────────────────────────────────────────────────────────
ZONE_COLORS = {
    "queue":(0,200,255), "service":(0,255,128), "exit":(200,80,255),
...
class CameraStream:
    ...
    def _on_inference(self, cam_id: int, frame: np.ndarray, raw_dets: list, inf_ms: float):
        ...
        if self.analytics.zones_defined():
            ...
            metrics = dict(self._EMPTY_METRICS, _tz={})

        # Pair 2 (Q2 + S2)
        if self.analytics_2.zones_defined():
            metrics_2 = self.analytics_2.update(raw_dets, w, h)
        else:
            metrics_2 = dict(self._EMPTY_METRICS, _tz={})

        tz  = metrics.pop("_tz", {})
        tz2 = metrics_2.pop("_tz", {})

        # Merge pair-2 metrics with q2_ prefix into main metrics dict
        metrics["q2_queue_length"]        = metrics_2.get("queue_length", 0)
        metrics["q2_avg_waiting_time"]    = metrics_2.get("avg_waiting_time", 0)
        metrics["q2_avg_processing_time"] = metrics_2.get("avg_processing_time", 0)
        metrics["q2_throughput_per_hour"]  = metrics_2.get("throughput_per_hour", 0)
        metrics["q2_wait_method"]         = metrics_2.get("wait_method", "none")
        metrics["q2_proc_method"]         = metrics_2.get("proc_method", "none")
        metrics["q2_queue_entries"]       = metrics_2.get("queue_entries", 0)
        metrics["q2_queue_exits"]         = metrics_2.get("queue_exits", 0)
        metrics["q2_service_exits"]       = metrics_2.get("service_exits", 0)
        metrics["q2_zone_counts"]         = metrics_2.get("zone_counts", {})
        metrics["q2_open_counter"]        = metrics_2.get("open_counter", False)
        metrics["q2_crossing_log"]        = metrics_2.get("crossing_log", [])
        metrics["q2_total_processed"]     = metrics_2.get("total_processed", 0)
        metrics["q2_L_queue_avg"]         = metrics_2.get("L_queue_avg", 0)
        metrics["q2_L_service_avg"]       = metrics_2.get("L_service_avg", 0)
        metrics["q2_lambda_exit_pm"]      = metrics_2.get("lambda_exit_pm", 0)
        metrics["q2_lambda_service_pm"]   = metrics_2.get("lambda_service_pm", 0)
        metrics["pair2_active"]           = self.analytics_2.zones_defined()

        ann = self._draw(frame.copy(), raw_dets, metrics, inf_ms, tz, tz2)
        self._cb_fc += 1
        now = time.time()
    ...
    def _draw(self, frame, dets, metrics, inf_ms, tz=None, tz2=None) -> np.ndarray:
        ...
        • Bounding boxes colored by zone (Q1=cyan, S1=green, Q2=orange, S2=lime)
        • Compact HUD strip at top — does NOT cover the video
        ...
        zone_colors = {
            "queue":     (0, 200, 255),   # cyan  — Q1
            "service":   (0, 230, 100),   # green — S1
            "queue_2":   (0, 165, 255),   # orange — Q2
            "service_2": (80, 255, 80),   # lime  — S2
        }
 
        # ── Draw zone polygons (Q1, S1, Q2, S2) ──────────────────────────────
        zone_draw_list = [
        ...
        for zone_key, poly, label in zone_draw_list:
            if poly and len(poly) >= 3:
                pts = np.array([[int(p[0] * w), int(p[1] * h)] for p in poly],
                                np.int32)
                overlay = frame.copy()
                cv2.fillPoly(overlay, [pts], zone_colors[zone_key])
                cv2.addWeighted(overlay, fill_alpha, frame, 1.0 - fill_alpha,
                                0, frame)
                cv2.polylines(frame, [pts], True, zone_colors[zone_key],
                ...
                cv2.putText(frame, label, (cx - 20, cy),
                            font, lbl_scale * 1.1,
                            zone_colors[zone_key], box_thick, cv2.LINE_AA)
        ...
        for det in dets:
            ...
            if zone == "queue":
                ...
                if box_h >= min_lbl_h:
                    cv2.putText(frame, "Q1", (x1 + 3, y1 + int(13 * draw_scale)),
                                font, lbl_scale,
                ...
                if box_h >= min_lbl_h:
                    cv2.putText(frame, "S1", (x1 + 3, y1 + int(13 * draw_scale)),
                                font, lbl_scale,
                                zone_colors["service"], box_thick, cv2.LINE_AA)
 
            elif zone2 == "queue":
                cv2.rectangle(frame, (x1, y1), (x2, y2),
                              zone_colors["queue_2"], box_thick)
                if box_h >= min_lbl_h:
                    cv2.putText(frame, "Q2", (x1 + 3, y1 + int(13 * draw_scale)),
                                font, lbl_scale,
                                zone_colors["queue_2"], box_thick, cv2.LINE_AA)
 
            elif zone2 == "service":
                cv2.rectangle(frame, (x1, y1), (x2, y2),
                              zone_colors["service_2"], box_thick)
                if box_h >= min_lbl_h:
                    cv2.putText(frame, "S2", (x1 + 3, y1 + int(13 * draw_scale)),
                                font, lbl_scale,
                                zone_colors["service_2"], box_thick, cv2.LINE_AA)
...
@app.route("/")
def index():
    return send_from_directory(str(_SCRIPT_DIR), "flow-lin.html")
...
def get_zones(cam_id:int):
    cam=manager.get(cam_id)
    if not cam: abort(404)
    a=cam.analytics; a2=cam.analytics_2
...
def clear_zones(cam_id:int):
    ...
    with cam._lock:
        cam._ann = None
        cam._metrics = {}
    log.info(f"[Cam-{cam_id}] Zones cleared — inference STOPPED, video resumed")
