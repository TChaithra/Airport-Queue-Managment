    ...
    def _worker(self, wid: int, cm):
        ...
        while self.running:
            ...
            try:
                ...
                if callback: callback(cam_id, dets, inf_ms)
            except Exception as e:
...
class _BTrack:
    ...
    def __init__(self, bbox, conf):
        ...
        self.state="tentative"; self.hits=1; self.age=0; self.tsu=0; self.bbox=list(bbox)
    def predict(self):
...
class ByteTracker:
    ...
    def update(self, dets):
        ...
        self.tracks = [t for t in self.tracks if not(t.state=="lost" and t.tsu>self.MAX_LOST)]
        return [{"bbox":t.bbox,"conf":t.conf,"cls":0,"track_id":t.id,"state":t.state}
...
# ─────────────────────────────────────────────────────────────────────────────
# Combined Analytics Engine — NO TRACKING
# Flow/Counting Method  +  Zone Flow Method  +  Little's Law
# ─────────────────────────────────────────────────────────────────────────────
#
...
#
#   FLOW/COUNTING METHOD (crossing detection without tracking):
#     Instead of following a specific person across a line,
#     we use ZONE COUNT CHANGES between consecutive frames:
#
#     Counter crossings (queue→service):
#       If queue count DROPS by N between frames → N persons left queue
#       counter_crossings += N
#
#     Exit crossings (service→exit):
#       If service count DROPS by N between frames → N persons left service
#       exit_crossings += N
#
#     Entry count (for context):
#       If queue count RISES by N → N persons entered queue
#
#     Smoothing: a 3-frame rolling window prevents noise spikes
#     (YOLO occasionally misses a detection for 1 frame — smoothing prevents
#      false crossings from single-frame detection gaps)
#
...
class QueueAnalytics:
    def __init__(self, alert_thr=10, wait_thr=300.0, cam_id=1):
        ...
        # ── Tripwire lines (stored but not used for tracking-based crossing) ──
        # Kept for dashboard display and future use
        self.counter_line: list = []
        self.exit_line:    list = []

        # ── Debounce crossing detector (replaces smoothed-delta) ────────────────
        ...
        # ── Flow counts — queue zone entries and exits ────────────────────────
        # Both derived purely from zone occupancy changes (no tripwire needed).
        # _queue_entries : queue count rose  (debounced) → person(s) entered queue
        # _queue_exits   : queue count dropped (debounced) → person(s) left queue
        # _service_exits : service count dropped (debounced) → person(s) left service
        ...
        # ── Consecutive empty-queue frame counter ─────────────────────────────
        # Tracks how many back-to-back frames have had L_queue_now == 0.
        # Once this exceeds _EMPTY_THRESH the wait time display is suppressed
        # (shown as "—") even when lambda_q > 0, because the rolling-average
        # L_queue is stale — the zone is visually empty right now.
        # At 15 fps, threshold=10 ≈ 0.67s of confirmed emptiness before hiding.
        ...
        # ── Wait / proc time history — used for fallback before first crossing ─
        # Stores valid Little's Law results so the fallback can use a
        # meaningful average instead of an arbitrary estimate.
        self._wait_history: deque = deque(maxlen=60)
        self._proc_history: deque = deque(maxlen=60)

        # ── Backward-compat ───────────────────────────────────────────────────
        self._tz: dict = {}  # {centroid_key: zone} for drawing color per bbox
    ...
    # ── Centroid helper ───────────────────────────────────────────────────────
    @staticmethod
    def _centroid(bbox, fw, fh):
        return (bbox[0] + bbox[2]) / 2 / fw, (bbox[1] + bbox[3]) / 2 / fh
    ...
    def update(self, dets, fw, fh):
        ...
        dets: raw YOLO bounding boxes after NMS — list of
              ...
              NO track_id needed. NO ByteTracker used.

        fw, fh: frame width and height in pixels
        """
        now = time.time()
        self._frame_count += 1
        self._tz = {}  # reset per-frame zone map for drawing

        # ── STEP 1: Count persons in each zone this frame (Zone Flow Method) ──
        L_queue_now   = 0
        L_service_now = 0

        for det in dets:
            cx, cy = self._centroid(det["bbox"], fw, fh)
            # Priority: service > queue (handle overlap at boundary)
            if _in_poly((cx, cy), self.zone_service):
                L_service_now += 1
                # Store for bbox coloring in _draw
                bbox_key = (int(det["bbox"][0]), int(det["bbox"][1]))
                self._tz[bbox_key] = "service"
            elif _in_poly((cx, cy), self.zone_queue):
                L_queue_now += 1
                bbox_key = (int(det["bbox"][0]), int(det["bbox"][1]))
                self._tz[bbox_key] = "queue"

        # Track consecutive empty frames per zone — used to suppress stale
        # rolling-average values after the zone empties.
        if L_queue_now == 0:
        ...
        # ── STEP 2 + 3: Debounce crossing detection (both zones) ──────────────
        #
        # A count change must persist for DEBOUNCE consecutive frames before
        # it is confirmed as a real crossing.  This replaces the smoothed-delta
        # approach that systematically undercounted single-person exits.
        #
        # Queue zone debounce
        ...
        if self._s_init_frames <= self.INIT_PHASE:
            ...
            if self._s_pend_frames >= self.DEBOUNCE:
                ...
                if s_delta < 0:
                    ...
                    # Service zone entries — not counted as a metric but
                    # update stable so subsequent drops are measured correctly.
                    self._s_stable = self._s_pend_val
        ...
        # Rolling avg occupancy (L in Little's Law)
        ...
        # λ for queue waiting time — exits are the most accurate signal;
        # fall back to entries only before any exit is observed.
        if lambda_exit > 0:
        ...
        # Empty-service suppression
        if self._empty_s_frames >= self._EMPTY_THRESH:
            avg_proc_s  = 0.0
            proc_method = "none"

        # Throughput = λ_service × 60 (persons per hour)
        throughput_hr = lambda_service * 60.0

        # ── Open counter signal ───────────────────────────────────────────────
        # Condition: wait is high AND queue has people AND no one has left yet.
        open_counter = (
...
def _build_gst_pipeline(src: str, is_stream: bool, render_dev: str) -> list:
    ...
    Your system has:
      vaapidecodebin  (rank 258, primary+2) — outputs VASurface → needs vaapipostproc
      vah264dec       (rank 0, explicit)    — outputs NV12 → videoconvert works directly

    Pipeline format for OpenCV CAP_GSTREAMER:
      source → parse → decode → convert → appsink
    ...
    if is_stream:
        ...
        # Local video files — qtdemux for MP4/MOV, parsebin as fallback
        ...
        src_auto = f'filesrc location="{src}" ! parsebin'

    # ── Pipeline 1: vaapidecodebin (H.264) ───────────────────────────────────
    # vaapidecodebin rank=258 — highest priority, auto-selected
    # VASurface → vaapipostproc converts to system NV12 → videoconvert to BGR
    if _GST_VAAPI_OK:
        pipelines.append((
    ...
    # ── Pipeline 2: vaapidecodebin (H.265) ───────────────────────────────────
    ...
    # ── Pipeline 3: vaapidecodebin without explicit postproc ─────────────────
    # Some versions of vaapidecodebin include postproc internally
    if _GST_VAAPI_OK:
        pipelines.append((
    ...
    # ── Pipeline 4: vah264dec (explicit, newer VA plugin) ────────────────────
    # rank=0 so must be named explicitly; outputs NV12 → videoconvert to BGR
    if _GST_VAAPI_OK:  # vah264dec ships with same plugin package
        pipelines.append((
            f'{src_h264} ! vah264dec ! '
            f'videoconvert ! video/x-raw,format=BGR ! appsink drop=1 sync=0',
            "GST-VA-H264"))

    # ── Pipeline 5: vaapidecodebin with decodebin for auto demux ─────────────
    # Works when container format is not MP4 or when qtdemux fails
    if _GST_VAAPI_OK and not is_stream:
        pipelines.append((
    ...
    # ── Pipeline 7: Full auto (GStreamer picks best decoder available) ────────
...
class VideoDecoder:
    ...
    Hardware decode priority:
      1. GStreamer + vaapidecodebin  (VA-API, Intel iGPU — lights up Video engine)
      2. GStreamer + vah264dec       (newer VA plugin, Intel Arc iGPU)
      3. GStreamer + msdkh264dec     (Intel QSV/MSDK)
      4. GStreamer + decodebin       (auto-selects)
      5. FFmpeg VA-API via OPENCV_FFMPEG_CAPTURE_OPTIONS
    ...
    def __init__(self):
        ...
        self._fc           = 0
        self._t0           = 0.0
        self.decode_method = "cpu"   # updated at open — shown in HUD + dashboard
        # Optional callback fired after every loop restart.
        # Set by CameraStream to call analytics.smooth_reset() so the
        # smoothing window is flushed at the loop boundary without resetting
        # cumulative counters — treating the file as a continuous stream.
    ...
    def _try_gst(self, pipeline: str, label: str):
        """
        Try to open a GStreamer pipeline for OpenCV.
        The pipeline must end with:
          videoconvert ! video/x-raw,format=BGR ! appsink drop=1 sync=0
        This gives OpenCV the BGR frames it expects directly.
        """
        try:
    ...
    def _try_ffmpeg(self, src, options: str, label: str):
        """Try FFmpeg-based open with hwaccel options."""
        try:
    ...
    def _open_best(self, src) -> Tuple[Optional[object], str]:
        """
        Try GPU decode methods in order.
        GStreamer methods actually use the GPU Video decode engine.
        FFmpeg methods are fallback (may silently use CPU despite hwaccel option).
        """
        src_str = str(src)
        ...
        # ── 1. GStreamer GPU pipelines (real hardware decode) ─────────────────
        if _GST_AVAILABLE:
        ...
        # ── 2. FFmpeg + VA-API explicit (may or may not use GPU) ─────────────
        if _VAAPI_OK:
    ...
    def _loop(self):
        src = self.source
        try: src = int(src)   # webcam index
        except: pass

        # Auto-detect local file → force recording mode for correct looping
        ...
        while self._run:
            ...
            if not ret:
                if self.mode == "recording":
                    # FIX: release and reopen instead of broken seek
                    loop_count += 1
                    ...
                    if native_fps <= 0 or native_fps > 120: native_fps = 25.0
                    frame_interval = 1.0 / native_fps
                    # Notify analytics to flush smoothing state only —
                    # counters and elapsed time are preserved so the loop
                    # is treated as a continuous stream.
...
class CameraStream:
    ...
    def __init__(self, name: str, source: str, mode: str="live",
        ...
        # NO ByteTracker — analytics takes raw YOLO detections directly
    ...
    def start(self, npu_engine: "NPUInferenceEngine"):
        ...
        # For recording (looping) mode, flush debounce pending state at
        # each loop boundary so the file plays as a continuous stream.
        ...
        t.start()
        log.info(f"[Cam-{self.cam_id}] '{self.name}' started (debounce crossings)")
    ...
    def _on_inference(self, cam_id: int, raw_dets: list, inf_ms: float):
        ...
        # ── QueueAnalytics — debounce crossings, both zones, all metrics ──────
        ...
        ann = self._draw(frame.copy(), raw_dets, metrics, inf_ms)
        self._cb_fc += 1
        now = time.time()
    ...
    def _draw(self, frame, dets, metrics, inf_ms) -> np.ndarray:
        """
        Clean minimal overlay:
        • NO zone polygons drawn here — dashboard canvas already shows them
        • NO tripwire lines drawn here — dashboard canvas already shows them
        • Only draw: bounding boxes (colored by zone) + compact HUD strip
        • HUD is a single slim bar at top — does NOT cover the video
        """
        h, w = frame.shape[:2]
        font = cv2.FONT_HERSHEY_SIMPLEX

        # ── Bounding boxes only — colored by zone, NO labels on gray boxes ──
        # Only draw a box if YOLO confidence is reasonable
        zone_colors = {
            "queue":   (0, 200, 255),   # cyan   — person in queue zone
            "service": (0, 230, 100),   # green  — person in service zone
        }
        # Bboxes inside queue zone (detected by QueueAnalytics._tz)
        for det in dets:
            x1, y1, x2, y2 = [int(v) for v in det["bbox"]]
            bbox_key = (x1, y1)
            zone = self.analytics._tz.get(bbox_key)
            if zone == "queue":
                # Queue zone person — cyan box + Q label
        ...
        wlbl = "L" if wm=="littles_law" else ("~" if wm=="estimated" else "—")
        pm   = metrics.get("proc_method","none")
        plbl = "L" if pm=="littles_law" else ("~" if pm=="estimated" else "—")
        ...
        awt_str = f"{awt:.0f}s" if wm not in ("none","no_data") else "—"
        hud_text = (
            f"Q:{ql} S:{sl} | "
            f"Wait[{wlbl}]:{awt_str}  Proc[{plbl}]:{apt:.0f}s | "
            f"In:{qin} Out:{qout} Svc:{scx}  Tput:{tph:.0f}/h | "
            f"{dm} {self._inf_fps:.0f}fps {inf_ms:.0f}ms"
            + (f" | {oc}" if oc else "")
        )

        # Measure text width to size the background strip
        (tw, th), _ = cv2.getTextSize(hud_text, font, 0.42, 1)
        strip_h = th + 10  # only as tall as the text needs
        ...
        # ── Alert banner — bottom center, small ──────────────────────────────
...
@app.route("/")
def index():
    return send_from_directory(str(_SCRIPT_DIR), "dashboard-hybrid.html")
...
def set_lines(cam_id:int):
    """Set tripwire lines for combined analytics."""
    cam=manager.get(cam_id)
    if not cam: abort(404)
    d=request.get_json(force=True)
